...
class MyXSpi(XSpi):
    ...
    def __init__(self):
        ...
        self.td.ServerInfo.ExtInfoChar128 = config.configs['td']['ExtInfoChar128']
        ...
        self.td.UserInfo.ExtInfoChar64 = config.configs['td']['ExtInfoChar64']
    ...
    def connect(self):
        ret = self.td.init(br'C:\Program Files\SmartQuant Ltd\OpenQuant 2014\XAPI\x86\Tdx\Tdx_Trade_x86.dll')
        if not ret:
    ...
    def sub_quote(self):
        ...
        for i in range(len(symbols_)):
            self.td.subscribe(symbols_[i], b'')
        print('订阅行情')

    def unsub_quote(self):
        symbols_ = pd.Series(self.symbols).str.encode('gbk')
        for i in range(len(symbols_)):
            self.td.unsubscribe(symbols_[i], b'')
        print('取消认阅')
    ...
    def wait_logined(self, sec=15, done_cnt=1):
        cnt = sec
